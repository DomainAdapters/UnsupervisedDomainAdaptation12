...
syn_Rod_train_Rot = MyDataset("./ROD-synROD/synROD/synARID_50k-split_sync_train1.txt",rotate = True, fold_name = "synROD")
syn_Rod_evaluation_Rot = MyDataset("./ROD-synROD/synROD/synARID_50k-split_sync_test1.txt", rotate = True, fold_name = "synROD")

rod_train_rot = MyDataset("./ROD-synROD/ROD/wrgbd_40k-split_sync.txt", rotate = True, fold_name = "ROD")
rod_evaluation_rot = MyDataset("./ROD-synROD/ROD/wrgbd_40k-split_sync.txt", rotate = True, fold_name = "ROD")
...
# # data sets
# syn_rod_train = DatasetGeneratorMultimodal(syn_rod_path, syn_rod_label_path)
# rod_set_evaluation = DatasetGeneratorMultimodal(syn_rod_path, syn_rod_label_path)

# rod_entropy_paper = DatasetGeneratorMultimodal(rod_path, rod_label_path)

# syn_rod_train_rot = DatasetGeneratorMultimodal(syn_rod_path, syn_rod_label_path, do_rot=True)
# syn_rod_evaluation_rot = DatasetGeneratorMultimodal(syn_rod_path, syn_rod_label_path, do_rot=True)

# rod_tain_rot = DatasetGeneratorMultimodal(rod_path, rod_label_path, do_rot=True)
...
for runs in RunBuilder.get_runs(parameters):
    ...
    if not resume:
      ...
      #print("ciao")
                                    # for obj_classifier_opt and rot_classifier_opt lr may need to multiply

    # Network architecture

    for epoch in range(first_epoch, first_epoch+5 ):
        ...
        print(epoch)
        # rgb, depth, label = syn_rod_train_loader_iter
        correct_syn_rod = 0
        correct_syn_rod_rot = 0
        correct_rod_rot = 0
        total_syn = 0
        total_rod = 0
        #tb.add_scalar("epoch ", epoch)

        # for i in range(0, 500, 62):
        for tmp in syn_rod_train_loader:
          ...
          with OptimizerManager(opt_lis):
                ...
                loss = class_loss + weight_entropy * loss_paper
                # print(class_loss)
                ...
                if weight_rot > 0.0:                    
                    ...
                    loss = rot_loss * weight_rot   # 0.5, 1
        ...
        #tb.add_graph(rgb_extrector, epoch)
        #tb.add_graph(depth_extrector)
        #tb.add_graph(obj_classifier)
        #tb.add_graph(rot_classifier)
...
# attention to name of file, put the same name as in above cell
#!cp giuseppe1.csv "drive/MyDrive/result_csv"

# here we save the model
rgb_save_name = 'rgb_extrector.pt'
path = F"./model_saved/{rgb_save_name}" 
torch.save( {'model_state_dict': rgb_extrector.state_dict()}, path)

depth_save_name = 'depth_extrector.pt'
path = F"./model_saved/{depth_save_name}" 
torch.save({'model_state_dict':depth_extrector.state_dict()}, path)

obj_save_name = 'obj_classifier.pt'
path = F"./model_saved/{obj_save_name}" 
torch.save({'model_state_dict':obj_classifier.state_dict()}, path)

rot_save_name = 'rot_classifier.pt'
path = F"./model_saved/{rot_save_name}" 
torch.save({'model_state_dict':rot_classifier.state_dict()}, path)


"""
!tensorboard dev upload --logdir /content/drive/MyDrive/logTensorBoard \
  --name "Prova" \
