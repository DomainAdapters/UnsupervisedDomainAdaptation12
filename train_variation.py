        ...
        for tmp in syn_rod_train_loader:
          ...
          with OptimizerManager(opt_lis):
                ...
                if weight_rot > 0.0:                    
                    ...
                    #pred_labels =   pred_labels.type(torch.float32) 
                    # pred_cos = pred_cos.type(torch.float32)
                    # pred_sin = pred_sin.type(torch.float32) 
                    ...
                    # cos_los = cos_los.type(torch.float32)
                    # sin_los = sin_los.type(torch.float32)
                    # print(loss)
...
rot_save_name = 'rot_regressor_var.pt'
path = F"./model_saved/{rot_save_name}" 
torch.save({'model_state_dict':Rot_regressor.state_dict()}, path)
